...
type CatalogProperty = {
  ...
  surface?: string;
  listedAt?: Date;
  owner?: { name?: string; profile?: string; phone?: string };
};
...
export default function Selection() {
  ...
  const catalogue: CatalogProperty[] = (publishedQuery.data ?? []).map((property, index) => ({
    ...
    description: property.description || "Une adresse publiée par un professionnel Kaba.",
  ...
  return <main className="selection-page" ref={pageRef}>
    ...
    {selected && <div className="detail-overlay" role="dialog" aria-modal="true"><div className="detail-panel"><button className="detail-close" onClick={() => setSelected(null)} aria-label="Fermer"><X size={19} /></button><img src={selected.image} alt={selected.title} /><div className="detail-panel-copy"><p className="eyebrow"><span>{selected.number}</span> Fiche de lieu</p><h2>{selected.title}</h2><p className="detail-location">{selected.location}</p><p className="detail-description">{selected.description}</p><strong>{selected.price}</strong><div className="legacy-property-stats detail-stats">{selected.bedrooms !== undefined && <span>🛏 {selected.bedrooms} chambres</span>}{selected.bathrooms !== undefined && <span>◉ {selected.bathrooms} salles de bain</span>}{selected.surface && <span>⌂ {selected.surface}</span>}{selected.views !== undefined && <span>◉ {selected.views} vues</span>}</div>{selected.owner?.name && <div className="legacy-owner detail-owner"><strong>{selected.owner.name}</strong>{selected.owner.profile && <span>{selected.owner.profile}</span>}{selected.owner.phone && <small>{selected.owner.phone}</small>}</div>}<form className="inquiry-form" onSubmit={submitInquiry}><label>Votre nom<input value={inquiryName} onChange={(event) => setInquiryName(event.target.value)} required placeholder="Nom et prénom" /></label><label>Email ou téléphone<input value={inquiryEmail} onChange={(event) => setInquiryEmail(event.target.value)} placeholder="Pour vous recontacter" /></label><label>Votre message<textarea value={inquiryMessage} onChange={(event) => setInquiryMessage(event.target.value)} placeholder="Je souhaite en savoir plus…" /></label><button className="contact-button" type="submit" disabled={createInquiry.isPending}>{createInquiry.isPending ? "Envoi…" : "Parler de cette adresse"} <ArrowUpRight size={16} /></button>{createInquiry.isSuccess && <p className="form-success">Votre demande a bien été envoyée.</p>}{createInquiry.error && <p className="form-error">Impossible d’envoyer la demande pour le moment.</p>}</form></div></div></div>}
  </main>;
}
