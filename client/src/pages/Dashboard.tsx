...
function Properties({ properties, loading }: { properties: Array<{ id: string; title: string; type: "Maison" | "Villa" | "Appartement" | "Terrain"; mode: "Vente" | "Location"; location: string; status: string; priceLabel: string; description?: string; isNew?: boolean; bedrooms?: number; bathrooms?: number; surface?: string; updatedAt: Date; media: DashboardMedia[] }>; loading: boolean }) {
  const [search, setSearch] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [title, setTitle] = useState("");
  const [propertyType, setPropertyType] = useState<"Maison" | "Villa" | "Appartement" | "Terrain">("Maison");
  const [propertyMode, setPropertyMode] = useState<"Vente" | "Location">("Vente");
  ...
  const resetForm = () => { setShowForm(false); setEditingId(null); setTitle(""); setPropertyType("Maison"); setPropertyMode("Vente"); setLocation(""); setPriceLabel(""); setDescription(""); setIsNew(false); setBedrooms(""); setBathrooms(""); setSurface(""); setUploadedMedia([]); setUploadMessage(""); setDragActive(false); };
