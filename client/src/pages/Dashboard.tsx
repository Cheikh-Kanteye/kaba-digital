...
export default function Dashboard() {
  ...
  const updateProfile = trpc.kaba.profile.update.useMutation({ onSuccess: () => profileQuery.refetch() });
  ...
  return <main className="dashboard-shell">
    <aside className={menuOpen ? "dashboard-sidebar is-open" : "dashboard-sidebar"}>
      <div className="dashboard-brand"><img src="/manus-storage/icon-kaba_e91dc42c.png" alt="Kaba" /><span>Kaba</span><small>ESPACE PRO</small></div>
    ...
    <section className="dashboard-main">
      ...
      {active === "Mon profil" && <Profile user={user} profile={profileQuery.data} loading={profileQuery.isLoading} updateProfile={updateProfile} />}
    </section>
  </main>;
}

function Overview({ properties, inquiriesCount, onAdd, loading }: { properties: Array<{ id: string; title: string; location: string; status: string; priceLabel: string; media: { url: string }[] }>; inquiriesCount: number; onAdd: () => void; loading: boolean }) {
  return <div className="dashboard-content"><div className="dashboard-welcome"><div><p className="eyebrow"><span>01</span> Votre espace</p><h2>Votre activité,<br /><em>en un regard.</em></h2></div><button className="dashboard-primary" onClick={onAdd}><Plus size={16} /> Ajouter un bien</button></div><div className="dashboard-stats"><div><span>Biens publiés</span><strong>{properties.filter((property) => property.status === "published").length}</strong><small>Données en direct</small></div><div><span>Biens enregistrés</span><strong>{properties.length}</strong><small>Votre catalogue Kaba</small></div><div><span>Demandes reçues</span><strong>{inquiriesCount}</strong><small>Depuis la mise en ligne</small></div><div><span>Profil</span><strong>OK</strong><small>Session sécurisée</small></div></div><div className="dashboard-section-head"><div><p className="eyebrow"><span>02</span> Vos adresses</p><h3>Biens récemment<br /><em>ajoutés.</em></h3></div><button className="dashboard-text-button" onClick={onAdd}>Voir tous les biens →</button></div>{loading ? <div className="dashboard-empty">Chargement de vos biens…</div> : properties.length === 0 ? <EmptyState title="Votre catalogue est vide" text="Ajoutez votre première adresse pour commencer." action={onAdd} /> : <div className="dashboard-property-grid">{properties.slice(0, 6).map((property) => <article className="dashboard-property-card" key={property.id}><div className="dashboard-property-image"><img src={property.media[0]?.url || "/manus-storage/kaba-hero-dakar_28cefd79.jpg"} alt={property.title} /><span className={`status status-${property.status}`}>{property.status}</span></div><div className="dashboard-property-copy"><h4>{property.title}</h4><p>{property.location}</p><span>{property.priceLabel}</span></div></article>)}</div>}</div>;
}

function Properties({ properties, loading }: { properties: Array<{ id: string; title: string; location: string; status: string; priceLabel: string; updatedAt: Date; media: { kind: string; url: string }[] }>; loading: boolean }) {
  const [search, setSearch] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [title, setTitle] = useState("");
  const [location, setLocation] = useState("");
  const [priceLabel, setPriceLabel] = useState("");
  const [imageUrls, setImageUrls] = useState("/manus-storage/kaba-hero-dakar_28cefd79.jpg");
  const [videoUrl, setVideoUrl] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [message, setMessage] = useState("");
  const utils = trpc.useUtils();
  const resetForm = () => { setShowForm(false); setEditingId(null); setTitle(""); setLocation(""); setPriceLabel(""); setImageUrls("/manus-storage/kaba-hero-dakar_28cefd79.jpg"); setVideoUrl(""); };
...
function Profile({ user, profile, loading, updateProfile }: { user: { name?: string | null; email?: string | null; role?: string }; profile?: { name?: string; email?: string; profile?: string; phone?: string; city?: string; rcNumber?: string; ninea?: string; agencyAddress?: string }; loading: boolean; updateProfile: { mutate: (input: Record<string, string>) => void; isPending: boolean } }) {
  const data = profile || {};
  return <div className="dashboard-content"><div className="profile-card"><div className="profile-card-top"><span className="dashboard-avatar large">{(data.name || user.name || user.email || "K").slice(0, 2).toUpperCase()}</span><div><p className="eyebrow"><span>04</span> Profil professionnel</p><h3>{data.name || user.name || "Membre Kaba"}</h3><p>{data.email || user.email || "Email non renseigné"}</p></div><span className="verified"><span>✓</span> Session vérifiée</span></div>{loading ? <div className="dashboard-empty">Chargement du profil…</div> : <div className="profile-fields"><label>Nom complet<input value={data.name || user.name || ""} readOnly placeholder="Non renseigné" /></label><label>Email professionnel<input value={data.email || user.email || ""} readOnly placeholder="Non renseigné" /></label><label>Profil<input value={data.profile || "Non renseigné"} readOnly /></label><label>Téléphone<input value={data.phone || ""} readOnly placeholder="Non renseigné" /></label><label>Ville<input value={data.city || ""} readOnly placeholder="Dakar" /></label><label>RC / NINEA<input value={[data.rcNumber, data.ninea].filter(Boolean).join(" · ")} readOnly placeholder="Non renseigné" /></label><label>Adresse d’agence<input value={data.agencyAddress || ""} readOnly placeholder="Non renseignée" /></label><button className="dashboard-primary" type="button" onClick={() => updateProfile.mutate({ name: data.name || user.name || "", profile: data.profile || "Agent immobilier", phone: data.phone || "", city: data.city || "", rcNumber: data.rcNumber || "", ninea: data.ninea || "", agencyAddress: data.agencyAddress || "" })}>{updateProfile.isPending ? "Synchronisation…" : "Synchroniser le profil"}</button></div>}</div></div>;
}
