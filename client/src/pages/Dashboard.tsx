...
export default function Dashboard() {
  ...
  return <main className="dashboard-shell">
    <aside className={menuOpen ? "dashboard-sidebar is-open" : "dashboard-sidebar"}>
      <div className="dashboard-brand"><img src="/manus-storage/icon-kaba_4ca3b0a4.png" alt="Kaba" /><span>Kaba</span><small>ESPACE PRO</small></div>
...
function Properties({ properties, loading }: { properties: Array<{ id: string; title: string; location: string; status: string; priceLabel: string; updatedAt: Date; media: DashboardMedia[] }>; loading: boolean }) {
  ...
  const [imageUrls, setImageUrls] = useState("/manus-storage/kaba-hero-dakar_73fcf37c.jpg");
  ...
  const resetForm = () => { setShowForm(false); setEditingId(null); setTitle(""); setLocation(""); setPriceLabel(""); setImageUrls("/manus-storage/kaba-hero-dakar_73fcf37c.jpg"); setVideoUrl(""); setUploadedMedia([]); setUploadMessage(""); };
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    try {
      ...
      setImageUrls((current) => [...current.split(/\n|,/).map((url) => url.trim()).filter(Boolean), ...uploaded.filter((item) => item.kind === "image").map((item) => item.url)].join("\n"));
